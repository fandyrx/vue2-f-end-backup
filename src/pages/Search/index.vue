            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOrder('1')">
                  <a >综合 <span v-show="isOne"><i v-show="isDesc">⬇</i><i v-show ="isAsc">⬆</i></span></a>
                </li>
                <li :class="{active:isTwo}"  @click="changeOrder('2')">
                  <a >销量 <span v-show="isTwo"><i v-show="isDesc">⬇</i><i v-show ="isAsc">⬆</i></span></a>
                </li>
              ...
              <li class="yui3-u-1-5" v-for="good in goodsList" :key="good.id">
                <div class="list-wrap">
                  ...
                  <div class="operate">
                    ...
                    <!-- 当前选择页  页面显示数据个数  总共数据  中间连续分页数-->
         <Pagination :pageNo="searchParams.pageNo" :pageSize="searchParams.pageSize"  :total="total" 
         :continues="5" @getPageNo="getPageNo" />
          <span>-------------------------------------------------------------下面测试</span>
          <hr>
         <test :pageNo="searchParams.pageNo" :pageSize="searchParams.pageSize"  :total="total" :continues="5" @getInfo="getInfo"/>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  beforeMount() {
    ...
    // es6 语法Object.assign('a','b','c')   将b,c对象合并到A，返回新对象A
  ...
  computed: {
    //mapstate返回是一个对象，mapState({'key':'value' ,key：‘value’简写，KV同名只写一个也行 })
    //计算属性也是对象，所以用拓展运算符解开再放入
    // ...mapState({
  ...
  methods: {
    ...
    removeCategoryName() {
      //清空query字符串，undefinend不会传给服务器，提高性能
    ...
    changeOrder(flag){
      //flag接受传递的参数，绑定的时候设计好了 综合传1，价格传2
